...
const SUPABASE_URL = "https://swlpnetfbbepduapkawc.supabase.co";
...
document.getElementById("signup").onclick = async () => {
  ...
  if (data.session) {
    ...
    alert(
      "Account created! If Supabase asks you to confirm your email, check your email first."
    );
  }
};
...
async function checkUser() {
  ...
  // Do NOT automatically open invite popups.
  // Invite links are handled only when needed.
}
...
async function loadServers() {
  ...
  for (const membership of memberships) {
    ...
    if (serverError || !server) continue;
...
async function checkInvite() {
  ...
  document.getElementById(
    ...
    // Automatically select the joined server
    const { data: server } =
      await supabase
        .from("servers")
        .select("*")
...
// Only check invite links when the URL actually
// contains ?invite=...
async function checkInviteIfPresent() {
